...
# def bandpass_filter(data:np.array, highpass_freq:float, lowpass_freq:float, sampling_freq:float, reset_mean=True):
#     """
#     Apply a symmetric bandpass filter to the input data.
...
#     # Convert cutoff frequencies to the Nyquist rate
#     highpass = highpass_freq / nyquist_rate
#     lowpass = lowpass_freq / nyquist_rate

#     # Design the FIR bandpass filter
#     numtaps = 101  # Number of filter coefficients (taps).
#     coeffs = firwin(numtaps, [highpass, lowpass], pass_zero=False)

#     # Apply the filter symmetrically (zero-phase filtering)
#     filtered_data = filtfilt(coeffs, 1.0, data)
#     if reset_mean == True:
#         delta = data.mean() - filtered_data.mean()
#         filtered_data += delta
#     return filtered_data
