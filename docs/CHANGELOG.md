...
`imgs/` held three PNGs committed in 2025-05/06. `grep -rn "imgs/"` across
every `.md` and `.py` returned **nothing** — they were referenced by no
document, regenerated by no script, and checked by no test. By 2026-09 they
predated the whole #26–#108 behaviour arc and nothing could tell. They are
replaced by three figures built from the library at `python examples/figures.py`.

Each `fig_*()` returns `(figure, facts)`, where `facts` is the set of numbers
the figure asserts visually. `test_figures.py` calls the same builders and
asserts the same numbers, so a library change that falsifies a figure fails a
test instead of leaving a stale PNG in the README. Two structural guards close
the loop the old set fell through: `test_no_orphaned_images` requires every PNG
in `imgs/` to come from a builder, and `test_every_figure_is_referenced_by_the_docs`
requires every builder's output to actually appear in README or EXAMPLES.

**`despiking.png`** — raw, despiked and the flagged samples on **one** y-axis.
Every panel in the old figures autoscaled independently, so "Raw" spanned −14
to 21 against "Despiked" at −8 to 8 and the reader could not separate the
correction from the rescale. The flagged samples are marked, because the
decision is what the signature feature actually makes.

**`gap_order.png`** — a dropout survives `filter_outliers()` untouched and the
Butterworth filters then refuse the series, with the real `ValueError` printed.
Panels 1 and 2 share a y-axis for the same reason.

**`band_power.png`** — the band, and `bin_fraction` drawn beside the ratio
rather than mentioned in a footnote. Built on a 60 s record: the 10 s signal
gives 0.1 Hz bins and draws its peaks as triangles, which is the artefact the
old FFT panels had.

### Fixed — a claim the first draft of `despiking.png` got backwards

That draft captioned the zoom panel "replaced by the fit". It is not. The
LOWESS fit *detects* — residuals from it drive the z-score — and the flagged
sample is replaced by linear interpolation between its nearest unflagged
neighbours, landing near the fit but never on it. Measured, not assumed: 0 of
31 replacements equal the fit and all 31 match the interpolation. Both are now
`facts` entries asserted by
`TestDespikingFigure::test_the_fit_detects_and_interpolation_replaces`, so the
distinction cannot quietly invert again.

Palette: categorical slots 1–3 of the reference data-viz palette, validated for
colour-vision-deficiency separation (worst adjacent pair ΔE 8.4, protan) and
≥ 3:1 contrast on a light surface. A darker aqua that looked safer measured
ΔE 5.5 and was rejected — the check is computed, not eyeballed. Series identity
never rests on colour alone: every series is in the legend and flagged samples
carry their own marker shape.

`imgs/` drops from 580 KB of unreferenced PNGs to 220 KB of referenced ones.
