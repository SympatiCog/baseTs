...
# Import modules
from LowessOutlierFilter import lowess_outlier_filter
from filters import bbf, bandpass_filter, sg_filter, interpolate_missing_values, lowpass_filter, highpass_filter
from plotting import plot_fft_power, plot, hist, plot_series, qc_plot
...
class baseTs(object):
    ...
    def interpto_samples(self, new_len:int, kind:str='linear', inplace:bool=False)->np.array:
        """
    ...
    def interp_to_uniform_grid(self, new_grid:np.array=None, kind:str='linear', inplace:bool=True):
        """
    ...
    def zscale(self, inplace:bool=False):
        """
        Z-scale the data.
        """
        if inplace:
    ...
    def normalize_range(self, inplace:bool=False):
        """
    ...
    def interpto_hz(self, new_freq:int, kind:str='linear', inplace:bool=False)-> np.array:
        """
    ...
    def trimto_timepoints(self, start_val:float=np.nan, end_val:float=-1, inplace = False)->np.array:
        """
    ...
    def lowess_detrend(self, frac:float=0.25, inplace:bool=False):
        """
    ...
    def highpass_at(self, cutoff, order:int=5, inplace:bool=False):
    ...
    def lowpass_at(self, cutoff, order:int=5, inplace:bool=False):
        nyq = 0.5 * self.freq  # Nyquist Frequency
        normal_cutoff = cutoff / nyq
    ...
    def interpolate_missing(self, inplace:bool=False):
        """
    ...
    def compute_fft_power(self, max_rate:float=np.nan, demean:bool=True, scale_power:bool=True):
        """
    ...
    def apply(self, func, *args, inplace=False, **kwargs):
    ...
    def to_dataframe(self, set_index:bool=False):
        """
